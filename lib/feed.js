...
function rss_2_0(options) {
    ...
    for(var i = 0; i < items.length; i++) {
         ...
         */
        if(items[i].source) {
            item.push({ source: [{ _attr: { url: items[i].source.url } }, items[i].source.name] });
        }
         ...
         */
        if(items[i].category) {
            if (items[i].category.name && items[i].category.domain) {
                item.push({ category: [{ _attr: { domain: items[i].category.domain } }, items[i].category.name] });
            } else {
                item.push({ category: items[i].category });
            }
        }
