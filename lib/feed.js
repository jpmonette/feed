...
function Feed(options) {
	...
	var item = {
	    ...
	    relatedLinks :    options.relatedLinks // Atom only
	}
...
function atom_1_0(options) {
	...
	// check if id property was set explicitly, otherwise default ID to the entry's link
	if (entries[i].id == undefined)
        ...
        var entry = [	
            ...
            { id:           entries[i].id },
            { link:         [{ _attr: { title: entries[i].title, href: entries[i].link } }]},
	...
	if (entries[i].relatedLinks && entries[i].relatedLinks.length > 0) {

	    for (var z = 0; z < entries[i].relatedLinks.length; z++) {
		var related = entries[i].relatedLinks[z];
		entry.push({ link: 
			     { _attr: 
			       { rel: 'related', 
				 href: related.link, 
