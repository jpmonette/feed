...
function Feed(options) {
    ...
    this.addNS = function(name, link) {
        this.customNS[name] = link;
    }
...
function atom_1_0(options) {
    ...
    for (var prop in options.customNS) {
        if (options.customNS.hasOwnProperty(prop)) {
            var a = {}; a[prop] = options.customNS[prop]; 
            feed.push({ _attr: a});
        }
    }
