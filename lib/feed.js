...
function rss_2_0(options) {
    ...
    if(options.title == undefined)
        return new Error('channel title is mandatory');
    else
        channel.push({ title: options.title });

    if(options.description == undefined)
        return new Error('channel description is mandatory');
    else
        channel.push({ description: options.description });

    if(options.link == undefined)
        return new Error('channel link is mandatory');
    else
        if(urlPattern.test(options.link))
            return new Error('invalid channel link');
        else
    ...
    if(author != undefined) {
        if(author.email && author.name)
            ...
            channel.push({ author: author.email });

        else if(!author.email && author.name)
            channel.push({ author: author.name });
    }
    ...
    for(var i = 0; i < items.length; i++) {
        ...
        if(items[i].title && items[i].description)
            return new Error('item must have at least a title or a description');
