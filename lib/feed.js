...
function atom_1_0(options) {
    ...
    if(options.link == undefined)
        ...
        if(!validateURL(options.link))
            throw new Error('invalid feed link' + options.link);
        else
    ...
    for(var i = 0; i < entries.length; i++) {
        ...
        if(entries[i].title == undefined)
            throw new Error('entry #' + i + ' need a title');

        if(entries[i].link == undefined)
            throw new Error('entry ' + entries[i].title + ' needs a link');
        else
        ...
        if(entries[i].date == undefined || !(entries[i].date instanceof Date))
            throw new Error('entry ' + entries[i].title + ' needs a date');
...
function rss_2_0(options) {
    ...
    if(options.link == undefined)
        ...
        if(!validateURL(options.link))
            throw new Error('invalid channel link: ' + options.link);
        else
