...
export default (ins: Feed) => {
  ...
  const base: any = {
    _declaration: { _attributes: { version: "2.0", encoding: "utf-8" } },
    rss: {
      ...
      channel: {
        ...
        language: { _text: options.language ? options.language : "en-us" },
    ...
    if (Array.isArray(entry.author)) {
      ...
      entry.author.map((author: Author) => {
        if (author.email && author.name) {
          item['dc:creator'].push({ _text: author.email + " (" + author.name + ")" });
        } else if (author.name) {
